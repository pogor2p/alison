...
(function() {
    ...
    const tourForm = document.querySelector('#tour-form__form');
    if (!tourForm) {
        return;
    }
    ...
    function restrictKeyboardInput(element) {
        element.addEventListener("keydown", function (e) {
            if (e.keyCode === 13) {
                tourForm.querySelector("#peopleCount").focus();
    ...
    let firstName = tourForm.querySelector("#firstName");
    stringLimitationOnInput(firstName, /[^a-zA-Z\s]+/g);

    //Last name validation
    let lastName = tourForm.querySelector("#lastName");
    stringLimitationOnInput(lastName, /[^a-zA-Z\s]+/g);

    //Email validation
    let userEmail = tourForm.querySelector("#userEmail");
    ...
    let countryCode = tourForm.querySelector("#countryCode");
    let phoneNumber = tourForm.querySelector("#telNum");
    ...
    //Calendar validation
    let checkInDate = tourForm.querySelector('#checkInDate');
    let checkOutDate = tourForm.querySelector('#checkOutDate');
